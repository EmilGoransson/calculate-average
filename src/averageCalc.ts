...
// @ts-expect-error
export function getAverageGPA(courses: Course[]): {
    average: number,
    ...
    if (courses.length != 0) {
        ...
        return {
            gradeHpWeight: Number(gradeHpWeight),
            hpUntilNow: Number(hpUntilNow),
            average: Number(gradeHpWeight / hpUntilNow),
            hpWithPGrade: hpWithPGrade
        }
